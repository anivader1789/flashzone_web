...
class Fakes {
  static DateTime generateFakeDate() => RandomDate.withRange(2022, 2024).random();

  static String generateFakeText({int limit = 20}) {
    List<String> result = List.empty(growable: true);
    for(int i = 0; i < limit; i++) {
      int code = random(Random(), 65, 122);
      result.add(String.fromCharCode(code));
    }
    final res = result.join();
    print("Generated post: $res");
    return res;
  }

  static Future<FZUser> generateFakeUser() async {
  ...
  static int random(Random r, int min, int max) => min + r.nextInt(max - min);
  static double randomDouble(Random r, num min, num max) => min + r.nextDouble() * (max - min);

  LatLng getRandomLocationAround({required double lon1, required double lat1, required double distance}) {
    ...
    return LatLng(lat: (lat + deltaLat) * 180 / pi, lon: (lon + deltaLon) * 180 / pi);
  }

  static Future<String?> _getAddressFromLatLng(LatLng coords) async {
    try {
...
class NamesStore {
  ...
  static List<String> maleFirstNames = [
          "Liam",
          "Noah",
          "Oliver",
          "Elijah",
  ...
  static List<String> femaleFirstNames = [
          "Olivia",
          "Emma",
          "Ava",
          "Charlotte",
